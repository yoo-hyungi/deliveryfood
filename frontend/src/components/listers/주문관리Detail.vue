<template>
    <v-card outlined>
        <v-card-title>
            주문관리 # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <String label="Store" v-model="item.store" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/주문관리/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
